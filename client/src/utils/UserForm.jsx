        ...
        <Form style={{ display: 'grid', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            <div>
              ...
              {touched.address?.street && errors.address?.street && <div style={{ color: 'red' }}>{errors.address.street}</div>}
            </div>
            <div>
              <label>City</label><br />
              <input name="address.city" value={values.address.city} onChange={handleChange} />
              {touched.address?.city && errors.address?.city && <div style={{ color: 'red' }}>{errors.address.city}</div>}
            </div>
            <div>
              <label>Zipcode</label><br />
              <input name="address.zipcode" value={values.address.zipcode} onChange={handleChange} />
              {touched.address?.zipcode && errors.address?.zipcode && <div style={{ color: 'red' }}>{errors.address.zipcode}</div>}
            </div>
          </div>
          ...
          <div>
            ...
            {touched.company?.name && errors.company?.name && <div style={{ color: 'red' }}>{errors.company.name}</div>}
          </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {values.skills.map((s, i) => (
                        <span key={i} style={{ border: '1px solid #ccc', padding: '4px 8px' }}>
                          {s}{' '}
                          <button type="button" onClick={() => helpers.remove(i)}>
                            ×
                          </button>
                        </span>
                      ))}
                  ...
                  <div>
                    <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const el = document.getElementById('newSlot');
                          if (el && el.value) {
                            helpers.push(el.value);
                            el.value = '';
                          }
